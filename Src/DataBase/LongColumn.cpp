...
namespace SimpleDB {
  ...
  void LongColumn::doBind(QueryPimplData& queryPimplData, int columnNumber) {
    SQLBindCol(queryPimplData.statementHandle, columnNumber, SQL_C_LONG,
	       &colValue, sizeof(long), &(getPimplData().lengthOrIndex));
    Column::doBind(queryPimplData, columnNumber);
  }

  std::ostream& LongColumn::oStream(std::ostream& oStream) {
    oStream << value();
    return oStream;
  }
}