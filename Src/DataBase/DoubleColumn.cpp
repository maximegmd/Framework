...
namespace SimpleDB {
  ...
  void DoubleColumn::doBind(QueryPimplData& queryPimplData, int columnNumber) {
    SQLBindCol(queryPimplData.statementHandle, columnNumber, SQL_C_DOUBLE,
	       &colValue, sizeof(double), &(getPimplData().lengthOrIndex));
    Column::doBind(queryPimplData, columnNumber);
  }

  std::ostream& DoubleColumn::oStream(std::ostream& oStream) {
    oStream << value();
    return oStream;
  }
}